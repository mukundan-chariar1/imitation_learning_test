...
class Discriminator(nn.Module):
    ...
    def setup(self):
        ...
        layers = []
        layers.append(nn.Dense(self.hidden_sizes[0]))
        for in_size, out_size in zip(self.hidden_sizes[:-1], self.hidden_sizes[1:]):
            layers.append(nn.Dense(out_size))
            if self.batchnorm:
                layers.append(nn.BatchNorm(use_running_average=False))
            layers.append(act())
            layers.append(nn.Dropout(0.2, deterministic=False))
